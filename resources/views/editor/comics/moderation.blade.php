...
@section('content')
    ...
    <div class="author_comics-form-container">
        <div class="author_comics-form-container-header">
            <h3>Модерация комикса: {{ $comic->name }}</h3>
        </div>
        ...
        <div class="lit-form-row">
            <div class="lit-fields-group">
                ...
                <div class="lit-field">
                    <label>Файл комикса</label>
                    @if (pathinfo($comic->comics_file, PATHINFO_EXTENSION) === 'pdf')
                        <div class="info-block">
                                ...
                                <div class="pdf-controls">
                                    ...
                                    <div class="zoom-controls">
                                        <button id="zoom-out" class="pdf-btn">
                                            <img src="{{ asset('images/icons/minus-icon-white.svg') }}" class="icon-20"
                                                 alt="icon">
                                        </button>
                                        <button id="zoom-in" class="pdf-btn">
                                            <img src="{{ asset('images/icons/plus-icon-white.svg') }}" class="icon-20"
                                                 alt="icon">
                                        </button>
                                    </div>
                                </div>
                            ...
                            <a href="{{ route('author_comic.download', $comic->slug) }}" class="primary-btn">
                                Скачать комикс
                                <img src="{{ asset('images/icons/download-icon-white.svg') }}" class="icon-24"
                                     alt="icon">
                            </a>
                            <input type="hidden" id="pdf-url" value="{{ asset($comic->comics_file) }}">
                        </div>
                    @else
                        <img src="{{ asset($comic->comics_file) }}" alt="Comic file" class="comic-image">
                    @endif
                </div>

                <div class="lit-field">
                    <label>Обложка комикса</label>
                    <img src="{{ $comic->cover ? Storage::url($comic->cover) : asset('images/default_template/comics.svg') }}"
                         alt="Comic cover" class="comic-image">
                </div>
            </div>
        </div>
